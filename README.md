...
We use `snake_case` naming conventions for the file system. Feel free to use your preferred naming convention if this does not fit your project.
...
- `npm run test`: This command will first lint the code using Oxlint and then run tests and report the test coverage using [c8](https://github.com/bcoe/c8).
...
## Oxlint and Oxfmt setup

The starter kit uses Oxlint for linting and Oxfmt for formatting.
Oxlint configuration is stored within the `.oxlintrc.json` file.
Oxfmt configuration is stored within the `.oxfmtrc.json` file.
You can customize both tools further by editing these files.
